    ...
    def cast(self):
        ...
        if self.showbase.rayQueue.getNumEntries() > 0 and self.showbase.isGUIopen == False:
                        ...
                        if hitObject.getPythonTag("data").get("tool_w") != None and self.showbase.mods_items.get(self.showbase.selectedBlockType) != None:
                            if self.showbase.mods_items[self.showbase.selectedBlockType]["data"].get("tool_w") != None and self.showbase.mods_items.get(self.showbase.selectedBlockType) != None:
                                if not self.showbase.mods_items[self.showbase.selectedBlockType]["data"].get("tool_w") >= hitObject.getPythonTag("data").get("tool_w"):
                                    self.showbase.print("Ce block DOIT se casser avec un outil avec une puissance de plus de :" + str(hitObject.getPythonTag("data").get("tool_w")))
            ...
            if hitObject.getPythonTag("type") == "zombie":
                ...
                self.showbase.print("Il reste"+str(life)+"de vie à ENDER GUARDIAN")
                if life > 0:
                    puissance = 2
                    if self.showbase.mods_items.get(self.showbase.selectedBlockType) != None:
                    ...
                    if self.showbase.enitiys.get("User")["data"]["quests"]["0x11111"] == False:
                        self.showbase.enitiys.get("User")["data"]["quests"]["0x11111"] = True
                        self.showbase.print("Vous AVEZ RÉUSSIS LE JEU!")
                    self.showbase.print("Vous avez gagné. Cependant, il vous arrive quelque chose, de terrible. Mais cela, vous ne le remarquer pas... Retours à la V2!")
                    self.showbase.world_saving.save_to_file_without_exit("lib")
                    try:
                        platform.linux_distribution()
                        os.system("python"+str(sys.version_info.major)+"."+str(sys.version_info.minor)+" main.py \""+sys.argv[1]+"\" &")
                        exit()
                    except: 
                        if platform.system() == "Windows":
                            os.system("start python"+str(sys.version_info.major)+"."+str(sys.version_info.minor)+" main.py \""+sys.argv[1]+"\"")
                            exit()
                        elif platform.system() == "Darwin":
                            os.system("open python"+str(sys.version_info.major)+"."+str(sys.version_info.minor)+" main.py \""+sys.argv[1]+"\"")
                            exit()

            elif hitObject.getPythonTag("type") == "pig":
                life = hitObject.getPythonTag("life")
                id = hitObject.getPythonTag("id")
                self.showbase.print("Il reste"+str(life)+"de vie à pig")
                if life > 0:
                    puissance = 2
                    if self.showbase.mods_items.get(self.showbase.selectedBlockType) != None:
                        if self.showbase.mods_items[self.showbase.selectedBlockType]["data"].get("strength") != None:
                ...
                if ObjectType != "bedrock":
                    ...
                    self.showbase.print("Vous ne pouvez pas casser la bedrock!")
                try:
                    if ObjectType != "bedrock":
                ...
                if self.showbase.blocks.get(hitNodePath.getName()) != None and ObjectType != "bedrock":
                    if ObjectType == "stone":
                        if self.showbase.enitiys.get("User")["data"]["quests"]["0x00001"] == False:
                            self.showbase.enitiys.get("User")["data"]["quests"]["0x00001"] = True
                            self.showbase.print("Vous avez réussis le succès Âge de la Stone!")
                    if ObjectType == "iron-ore":
                        if self.showbase.enitiys.get("User")["data"]["quests"]["0x00011"] == False:
                            self.showbase.enitiys.get("User")["data"]["quests"]["0x00011"] = True
                            self.showbase.print("Vous avez réussis le succès Âge du Fer!")
                    if ObjectType == "diamond-ore":
                        if self.showbase.enitiys.get("User")["data"]["quests"]["0x00111"] == False:
                            self.showbase.enitiys.get("User")["data"]["quests"]["0x00111"] = True
                            self.showbase.print("Vous avez réussis le succès Âge du Diamant!")
                    trne = 0
