...
class User_Gravity(ShowBase):
    ...
    def is_under_block(self):
        ...
        for zombieUUID in self.showbase.zombiesUUID:
            if self.showbase.zombies.get(str(zombieUUID)+"_zombie") != None:
                ...
                if zombie.getTransform().getPos().getX()-5 < userX and zombie.getTransform().getPos().getX()+5 > userX:
                    if zombie.getTransform().getPos().getY()-5 < userY and zombie.getTransform().getPos().getY()+5 > userY:
                        if zombie.getTransform().getPos().getZ()-5 < userZ and zombie.getTransform().getPos().getZ()+5 > userZ:
                            self.showbase.userLife -= 18
                            self.showbase.print("Il vous reste" + str(self.showbase.userLife) + "de vie")
    ...
    def update_user_to_shape(self):
        ...
        self.showbase.enitiys["User"]["data"]["life"] = self.showbase.userLife