    ...
    def cast(self):
        ...
        if self.showbase.rayQueue.getNumEntries() > 0 and self.showbase.isGUIopen == False:
                ...
                if self.showbase.is_breaking_block == False:
                    ...
                    if (hitObject.getPythonTag("data_content") != self.showbase.witch_block or time.time() < self.showbase.break_timer):
                        ...
                        if hitObject.getPythonTag("data").get("tool_w") != None:
                            if self.showbase.mods_items[self.showbase.selectedBlockType]["data"].get("tool_w") != None:
            ...
            if hitObject.getPythonTag("type") == "zombie":
                ...
                self.showbase.print("Il reste"+str(life)+"de vie à zombie")
                if life > 0:
                    puissance = 2
                    if self.showbase.mods_items.get(self.showbase.selectedBlockType) != None:
            ...
            def AbleToHit():
                self.showbase.isAbleToHit = True
            if hitObject.getPythonTag("type") == "player":
                if self.showbase.isAbleToHit == True:
                    id = hitObject.getPythonTag("id")
                    self.showbase.timeline_control("", "__hit__"+str(id)+"__hit__")
                    self.showbase.isAbleToHit = False
                    setTimeout(AbleToHit, 1000)
                ...
                try:
                    if ObjectType != "bedrock":
                        self.showbase.timeline_control("bblock", "_IFO_DAT__CBX_"+str(Pos[0])+"_CBX_"+"_CBY_"+str(Pos[1])+"_CBY_"+"_CBZ_"+str(Pos[2])+"_CBZ__IFO_DAT_")
